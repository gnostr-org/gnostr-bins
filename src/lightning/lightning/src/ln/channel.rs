	...
	SP::Target: SignerProvider,
{
		...
		SP::Target: SignerProvider,
	{
		...
		let commitment_tx_fee = commit_tx_fee_msat(
			open_channel_fields.commitment_feerate_sat_per_1000_weight,
	...
	SP::Target: SignerProvider,
{
	...
	pub fn accept_channel(
		...
		if msg.common_fields.dust_limit_satoshis
		...
		if msg.common_fields.htlc_minimum_msat >= full_channel_value_msat {
			return Err(ChannelError::Close(format!(
				...
				msg.common_fields.htlc_minimum_msat, full_channel_value_msat
			)));
		}
		...
		if msg.common_fields.max_accepted_htlcs > MAX_HTLCS {
			return Err(ChannelError::Close(format!(
				"max_accepted_htlcs was {}. It must not be larger than {}",
				msg.common_fields.max_accepted_htlcs, MAX_HTLCS
			)));
		}
		...
		if msg.common_fields.htlc_minimum_msat > peer_limits.max_htlc_minimum_msat {
			return Err(ChannelError::Close(format!(
				"htlc_minimum_msat ({}) is higher than the user specified limit ({})",
				msg.common_fields.htlc_minimum_msat, peer_limits.max_htlc_minimum_msat
			)));
		}
			...
			< peer_limits.min_max_htlc_value_in_flight_msat
		{
			return Err(ChannelError::Close(format!(
				"max_htlc_value_in_flight_msat ({}) is less than the user specified limit ({})",
				msg.common_fields.max_htlc_value_in_flight_msat,
				peer_limits.min_max_htlc_value_in_flight_msat
			)));
		}
		...
		if msg.common_fields.max_accepted_htlcs < peer_limits.min_max_accepted_htlcs {
			return Err(ChannelError::Close(format!(
				"max_accepted_htlcs ({}) is less than the user specified limit ({})",
				msg.common_fields.max_accepted_htlcs, peer_limits.min_max_accepted_htlcs
			)));
		}
		if msg.common_fields.dust_limit_satoshis < MIN_CHAN_DUST_LIMIT_SATOSHIS {
			return Err(ChannelError::Close(format!(
				"dust_limit_satoshis ({}) is less than the implementation limit ({})",
				msg.common_fields.dust_limit_satoshis, MIN_CHAN_DUST_LIMIT_SATOSHIS
			)));
		}
		if msg.common_fields.dust_limit_satoshis > MAX_CHAN_DUST_LIMIT_SATOSHIS {
			return Err(ChannelError::Close(format!(
				"dust_limit_satoshis ({}) is greater than the implementation limit ({})",
				msg.common_fields.dust_limit_satoshis, MAX_CHAN_DUST_LIMIT_SATOSHIS
			)));
		}
		...
		self.context.counterparty_max_htlc_value_in_flight_msat = cmp::min(
			msg.common_fields.max_htlc_value_in_flight_msat,
			self.context.channel_value_satoshis * 1000,
		);
		...
		self.context.counterparty_cur_commitment_point =
			Some(msg.common_fields.first_per_commitment_point);
