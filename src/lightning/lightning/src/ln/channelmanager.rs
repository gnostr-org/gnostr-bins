...
pub use crate::ln::channel::{
	InboundHTLCDetails, InboundHTLCStateDetails, OutboundHTLCDetails, OutboundHTLCStateDetails,
};
...
use crate::offers::invoice::{
	BlindedPayInfo, Bolt12Invoice, DerivedSigningPubkey, ExplicitSigningPubkey, InvoiceBuilder,
				...
				let response = invoice
					...
					.and_then(|payment_id| {
						...
						if invoice.invoice_features().requires_unknown_bits_from(&features) {
							...
							self.send_payment_for_bolt12_invoice(&invoice, payment_id).map_err(
								|e| {
