...
pub(crate) fn maybe_add_change_output(
	tx: &mut Transaction, input_value: u64, witness_max_weight: u64,
...
mod tests {
	...
	fn test_tx_value_overrun() {
		...
		let mut tx = Transaction {
			...
			input: Vec::new(),
			output: vec![TxOut { script_pubkey: ScriptBuf::new(), value: 1000 }],
		};
		assert!(maybe_add_change_output(&mut tx, 21_000_000_0000_0001, 0, 253, ScriptBuf::new())
			.is_err());
