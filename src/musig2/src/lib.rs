...
pub fn sign<C: bitcoin::secp256k1::Signing + bitcoin::secp256k1::Verification>(
    secret_key: &SecretKey,
    secret_nonce: &SecretNonce,
    aggregate_nonce: &PublicNonce,
    keys: &SignerPublicKeys,
    message: &[u8],
    ...
    let (effective_nonce, nonce_coefficient) = calculate_final_nonce(
        aggregate_nonce,
    ...
    // m*a*d
    let summand_a = final_secret_key
        .mul_tweak(&m)
        .unwrap()
        .mul_tweak(&a)
        .unwrap();
    ...
    // s = m*a*d + k1 + b*k2
    let s = summand_a
        .add_tweak(&Scalar::from(summand_b))
        .unwrap()
        .add_tweak(&Scalar::from(final_secret_nonce.0))
        .unwrap();
    let is_valid = verify_partial_signature(
        &PartialSignature(s),
        &secret_nonce.to_public(&secp_context),
        aggregate_nonce,
        &secret_key.public_key(&secp_context),
        keys,
        message,
        secp_context,
    );
...
pub fn aggregate_partial_signatures<K: Borrow<PartialSignature>>(
    partial_signatures: &[K],
    final_nonce: &PublicKey,
) -> Signature {
    let mut aggregate_signature = Scalar::ZERO;
    for current_signature in partial_signatures {
        aggregate_signature = Scalar::from(
            current_signature
                .borrow()
                .0
                .add_tweak(&aggregate_signature)
                .unwrap(),
        );
    }
...
pub fn verify_partial_signature<
    C: bitcoin::secp256k1::Verification + bitcoin::secp256k1::Signing,
>(
    partial_signature: &PartialSignature,
    partial_public_nonce: &PublicNonce,
    aggregate_public_nonce: &PublicNonce,
    ...
    let ordered_keys = keys.ordered_keys();
    verify_partial_signature_with_ordered_keys(
        partial_signature,
        partial_public_nonce,
        aggregate_public_nonce,
        partial_public_key,
        &ordered_keys,
        message,
        secp_context,
...
fn verify_partial_signature_with_ordered_keys<
    K: Borrow<PublicKey>,
    C: bitcoin::secp256k1::Verification + bitcoin::secp256k1::Signing,
>(
    partial_signature: &PartialSignature,
    ...
    let partial_nonce_summand = partial_public_nonce
        .1
        .mul_tweak(&secp_context, &nonce_coefficient)
        .unwrap();
    let mut effective_partial_nonce =
        PublicKey::combine_keys(&[&partial_public_nonce.0, &partial_nonce_summand]).unwrap();
    ...
    let summand = final_public_key
        .mul_tweak(&secp_context, &a)
        .unwrap()
        .mul_tweak(&secp_context, &m)
        .unwrap();
...
mod tests {
    ...
    fn test_signature_aggregation() {
        ...
        for current_pubkey_hex in pubkey_hexes {
            let current_key =
                PublicKey::from_slice(&hex::decode(current_pubkey_hex).unwrap()).unwrap();
        ...
        for current_nonce_hex in public_nonce_hexes {
            let current_nonce =
                PublicNonce::from_slice(&hex::decode(current_nonce_hex).unwrap()).unwrap();
        ...
        for current_signature_hex in partial_signature_hexes {
            let current_signature =
                SecretKey::from_slice(&hex::decode(current_signature_hex).unwrap()).unwrap();
        ...
        let message =
            hex::decode("599c67ea410d005b9da90817cf03ed3b1c868e4da4edf00a5880b0082c237869")
                .unwrap();
        ...
        let secp_context = Secp256k1::new();
        {
            ...
            println!(
                "aggregate pubkey (Q): {}",
                hex::encode(aggregate_public_key.key.serialize())
            );

            let (final_nonce, _) = calculate_final_nonce(
                &aggregate_nonce,
                &aggregate_public_key,
                &message,
                &secp_context,
            ...
            let signature =
                aggregate_partial_signatures(partial_signatures.as_slice(), &final_nonce);
        ...
        }
        {
            ...
            println!(
                "aggregate pubkey (Q, reversed order): {}",
                hex::encode(aggregate_public_key.key.serialize())
            );
        ...
        }
        {
            ...
            let reversed_public_keys =
                SignerPublicKeys::Ordered(vec![public_keys[1], public_keys[0]]);
            let aggregate_public_key = aggregate_keys(&reversed_public_keys, &secp_context);
            println!(
                "aggregate pubkey (Q, reversed order): {}",
                hex::encode(aggregate_public_key.key.serialize())
    ...
    fn test_signature_verification() {
        ...
        let secret_keys = vec![
            SecretKey::from_slice(
                &hex::decode("d2dc6f5df7c56acf38c7fa0ae7a759ae30e19b37359dfde015872324c7ef6e05")
                    .unwrap(),
            )
            .unwrap(),
            SecretKey::from_slice(
                &hex::decode("935f972da013f80ae011890fa89b67a27b7be6ccb24d3274d18b2d4067f261a9")
                    .unwrap(),
            )
            .unwrap(),
        ];
        ...
        let secret_nonce_hexes = vec![
            vec![
                "00a32f8548f59c533f55db9754e3c0ba3c2544f085649fdce42b8bd3f244c2ca",
                "0384449bed61004e8863452a38534e91875516c3cc543122ce2be1f318450255",
            ],
            vec![
                "f66b072a869bc2a57d776d487151d707e82b4f1b885066a589858c1bf3871db6",
                "03ed391c9658ab6031a96acbd5e2d9fec465efdc8c0d0b765c9b9f3579d520fb",
            ],
        ];
        let mut secret_nonces = Vec::with_capacity(secret_nonce_hexes.len());
        for current_nonce_hex in secret_nonce_hexes {
            let nonce_a =
                SecretKey::from_slice(&hex::decode(current_nonce_hex[0]).unwrap()).unwrap();
            let nonce_b =
                SecretKey::from_slice(&hex::decode(current_nonce_hex[0]).unwrap()).unwrap();
        ...
        let message =
            hex::decode("599c67ea410d005b9da90817cf03ed3b1c868e4da4edf00a5880b0082c237869")
                .unwrap();
        let (effective_nonce, _) =
            calculate_final_nonce(&aggregate_nonce, &aggregate_pubkey, &message, &secp_context);

        let partial_signature_a = sign(
            &secret_keys[0],
            &secret_nonces[0],
            &aggregate_nonce,
            &sortable_public_keys,
            &message,
            &secp_context,
        );
        let partial_signature_b = sign(
            &secret_keys[1],
            &secret_nonces[1],
            &aggregate_nonce,
            &sortable_public_keys,
        ...
        let is_signature_a_valid = verify_partial_signature(
            &partial_signature_a,
            &public_nonces[0],
            &aggregate_nonce,
            &public_keys[0],
        ...
        let is_signature_b_valid = verify_partial_signature(
            &partial_signature_b,
            &public_nonces[1],
            &aggregate_nonce,
            &public_keys[1],
        ...
        let schnorr_signature = aggregate_partial_signatures(
            &[partial_signature_a, partial_signature_b],
            &effective_nonce,
        );
        let is_schnorr_signature_valid = verify_schnorr_signature(
            &schnorr_signature,
            &PublicKey::from_x_only_public_key(aggregate_pubkey.key, Parity::Even),
            &message,
        );
    ...
    fn test_signature_verification_against_vectors() {
        ...
        let secret_nonce_a =
            SecretKey::from_slice(&hex::decode(&secret_nonce_hex[..64]).unwrap()).unwrap();
        let secret_nonce_b =
            SecretKey::from_slice(&hex::decode(&secret_nonce_hex[64..]).unwrap()).unwrap();
        println!(
            "nonce A pubkey: {}",
            hex::encode(secret_nonce_a.public_key(&secp_context).serialize())
        );
        println!(
            "nonce B pubkey: {}",
            hex::encode(secret_nonce_b.public_key(&secp_context).serialize())
        );
        ...
        let aggregate_nonce =
            PublicNonce::from_slice(&hex::decode(aggregate_nonce_hex).unwrap()).unwrap();

        let message = hex::decode(message_hex).unwrap();

        let ordered_public_keys = SignerPublicKeys::Ordered(public_keys);
        let partial_signature = sign(
            &secret_key,
            &secret_nonce,
            &aggregate_nonce,
            &ordered_public_keys,
        ...
        assert_eq!(
            hex::encode(&partial_signature.0.secret_bytes()),
        ...
        let is_signature_valid = verify_partial_signature(
            &partial_signature,
            &secret_nonce.to_public(&secp_context),
            &aggregate_nonce,
            &secret_key.public_key(&secp_context),
