...
impl<'a, C: Context> Reader<'a, C> for BufferReader<'a, C> {
    ...
    #[inline(always)]
    fn read_bytes_borrowed_from_reader<'r>(
        &'r mut self,
        length: usize,
    ) -> Option<Result<&'r [u8], C::Error>> {
        if self.can_read_at_least(length) == Some(false) {
            return Some(Err(error_end_of_input()));
        }
...
impl<'ctx, 'r, 'a, C: Context> Reader<'r, C> for CopyingBufferReader<'ctx, 'a, C> {
    ...
    #[inline(always)]
    fn read_bytes_borrowed_from_reader<'reader>(
        &'reader mut self,
        length: usize,
    ) -> Option<Result<&'reader [u8], C::Error>> {
        if self.can_read_at_least(length) == Some(false) {
            return Some(Err(error_end_of_input()));
        }
        ...
        #[cfg(all(target_os = "linux", target_arch = "x86_64"))]
        {
            ...
            impl std::ops::Deref for Mmap {
                ...
                fn deref(&self) -> &Self::Target {
                    unsafe { std::slice::from_raw_parts(self.0.cast::<u8>(), self.1) }
                }
            }
