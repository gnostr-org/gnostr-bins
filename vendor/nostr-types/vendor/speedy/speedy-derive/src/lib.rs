                ...
                syn::PathArguments::AngleBracketed(syn::AngleBracketedGenericArguments {
                    ...
                    args.iter().all(|arg| {
                        match arg {
                            ...
                            syn::GenericArgument::AssocType(assoc_type) => {
                                is_guaranteed_non_recursive(&assoc_type.ty)
                            }
                            // TODO: How to handle these?
...
enum ToplevelStructAttribute {
    UnsafeIsPrimitive {
        key_token: kw::unsafe_is_primitive,
        kind: IsPrimitive,
    },
...
fn check_is_primitive(
    input: &syn::DeriveInput,
    structure: &Struct,
    attrs: &ToplevelStructAttributes,
) -> IsPrimitiveResult {
    ...
    let can_be_primitive = is_forced
        || structure
            .fields
            .iter()
    ...
    IsPrimitiveResult {
        is_ty_packed,
        is_ty_transparent,
        is_ty_potentially_primitive: can_be_primitive
            && (is_forced
                || is_ty_transparent
                || is_ty_packed
                || is_ty_c
        ...
        syn::Data::Struct(syn::DataStruct { ref fields, .. }) => {
            ...
            let impl_primitive = if is_primitive.is_ty_transparent {
                ...
                let is_primitive = generate_is_primitive(
                    &structure.fields,
                    false,
                    is_primitive.check_order,
                    is_primitive.is_forced,
                );
            ...
            let impl_zerocopyable = if is_primitive.impl_zerocopyable {
                let (impl_params, ty_params, where_clause) = common_tokens(
                    ...
                    &field_types,
                    Trait::ZeroCopyable {
                        is_packed: is_primitive.is_ty_packed,
                        is_forced: is_primitive.is_forced,
                    },
                );
                quote! {
