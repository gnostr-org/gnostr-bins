...
impl SecretNonce {
    /// Convert to public
    pub fn to_public<C: bitcoin::secp256k1::Signing>(
        &self,
    ...
    pub fn from_slice(data: &[u8]) -> Result<SecretNonce, bitcoin::secp256k1::Error> {
        assert_eq!(
            data.len(),
            64,
            "SecretNonce can only be read from exactly 64 bytes"
        );
...
impl PublicNonce {
    ...
    pub fn from_slice(data: &[u8]) -> Result<PublicNonce, bitcoin::secp256k1::Error> {
        assert_eq!(
            data.len(),
            66,
            "PublicNonce can only be read from exactly 66 bytes"
        );
...
impl PartialSignature {
    ...
    pub fn from_slice(data: &[u8]) -> Result<PartialSignature, bitcoin::secp256k1::Error> {
        assert_eq!(
            data.len(),
            32,
            "PartialSignature can only be read from exactly 32 bytes"
        );
