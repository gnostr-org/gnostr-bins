...
use crate::{tagged_hash_scalar, AggregateKey, PublicNonce};
...
pub(crate) fn calculate_final_nonce<C: bitcoin::secp256k1::Verification>(
    aggregate_nonce: &PublicNonce,
    aggregate_public_key: &AggregateKey,
    message: &[u8],
    secp_context: &Secp256k1<C>,
) -> (PublicKey, Scalar) {
    let nonce_coefficient =
        nonce_linear_combination_coefficient(&aggregate_public_key.key, &aggregate_nonce, message);
    let nonce_summand = aggregate_nonce
        .1
...
fn nonce_linear_combination_coefficient(
    aggregate_public_key: &XOnlyPublicKey,
    aggregate_nonce: &PublicNonce,
    message: &[u8],
) -> Scalar {
    tagged_hash_scalar(
        "MuSig/noncecoef",
        &[
            &aggregate_nonce.serialize(),
            &aggregate_public_key.serialize(),
            message,
        ],
    )
}
