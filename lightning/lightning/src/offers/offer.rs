...
use crate::offers::invoice_request::{
	InvoiceRequestWithDerivedPayerIdBuilder, InvoiceRequestWithExplicitPayerIdBuilder,
};
...
macro_rules! offer_derived_metadata_builder_methods {
	($secp_context: ty) => {
		...
		pub fn deriving_signing_pubkey<ES: Deref>(
			description: String, node_id: PublicKey, expanded_key: &ExpandedKey,
			entropy_source: ES, secp_ctx: &'a Secp256k1<$secp_context>,
		) -> Self
		where
