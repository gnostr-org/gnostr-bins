...
impl TaggedHash {
	...
	pub(super) fn from_tlv_stream<'a, I: core::iter::Iterator<Item = TlvRecord<'a>>>(
		...
		let digest =
			Message::from_slice(tagged_hash(tag_hash, merkle_root).as_byte_array()).unwrap();
...
#[cfg(test)]
mod tests {
	use super::{TlvStream, WithoutSignatures, SIGNATURE_TYPES};
	...
	fn calculates_merkle_root_hash() {
		...
		assert_eq!(
			super::root_hash(TlvStream::new(
				&<Vec<u8>>::from_hex(concat!(tlv1!(), tlv2!())).unwrap()
			)),
			sha256::Hash::from_slice(
				&<Vec<u8>>::from_hex(
		...
		assert_eq!(
			super::root_hash(TlvStream::new(
				&<Vec<u8>>::from_hex(concat!(tlv1!(), tlv2!(), tlv3!())).unwrap()
			)),
			sha256::Hash::from_slice(
				&<Vec<u8>>::from_hex(
	...
	fn compute_tagged_hash() {
		...
		let actual_digest =
			Message::from_slice(super::tagged_hash(tag, tagged_hash.merkle_root()).as_byte_array())
				.unwrap();
